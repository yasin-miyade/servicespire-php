...
<body class="bg-gray-100 font-inter p-6">
    ...
    <!-- Helper Profile Modal - Enhanced Professional Design -->
    <div id="helper-profile-modal" class="fixed inset-0 bg-black bg-opacity-60 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            <div class="flex justify-between items-center p-4 border-b sticky top-0 bg-white z-10">
                <h3 class="text-lg font-semibold text-gray-800">Helper Profile</h3>
                <button onclick="closeHelperProfile()" class="text-gray-500 hover:text-gray-800 transition focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </div>
            <div id="helper-profile-content" class="max-h-[80vh] overflow-y-auto">
                <div class="flex flex-col items-center justify-center p-8 bg-gray-50">
                    <div class="w-16 h-16 mb-4">
                        <div class="animate-spin rounded-full h-full w-full border-t-2 border-b-2 border-blue-500"></div>
                    </div>
                    <p class="text-gray-500">Loading helper profile...</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function hideNotification(notificationId) {
            ...
            $("#notification-" + notificationId).fadeOut(500, function() {
                $(this).hide();
        ...
        function showHelperProfile(helperId) {
            document.getElementById('helper-profile-modal').style.display = 'flex';
            
            const modalContent = document.querySelector('#helper-profile-modal > div');
            ...
            $.ajax({
                url: "get_helper_profile.php",
                type: "GET",
                data: { helper_id: helperId },
                success: function(response) {
                    $("#helper-profile-content").html(response);
                },
                error: function() {
                    $("#helper-profile-content").html(`
                        <div class="p-8 text-center">
                            ...
                            <p class="text-gray-500">We couldn't load the helper profile. Please try again.</p>
                        </div>
                    `);
                }
            });
        ...
        function closeHelperProfile() {
            const modalContent = document.querySelector('#helper-profile-modal > div');
            modalContent.style.opacity = '0';
            modalContent.style.transform = 'scale(0.9)';
            
            setTimeout(() => {
                document.getElementById('helper-profile-modal').style.display = 'none';
                $("#helper-profile-content").html(`
                    <div class="flex flex-col items-center justify-center p-8 bg-gray-50">
                        <div class="w-16 h-16 mb-4">
                            <div class="animate-spin rounded-full h-full w-full border-t-2 border-b-2 border-blue-500"></div>
        ...
        $(document).ready(function () {
            ...
            $('#helper-profile-modal').click(function(e) {
                if (e.target.id === 'helper-profile-modal') {
                    closeHelperProfile();
                }
            });
        });
